...
// Enhanced file transfer server functions
namespace FileTransferServer {

#ifdef _WIN32
void handle_enhanced_file_transfer_server_windows() {
    SOCKET listenSocket, clientSocket;
    sockaddr_in serverAddr, clientAddr;
    int clientAddrSize = sizeof(clientAddr);
    ...
    std::cout << "Enhanced TCP Server listening for file transfers on port " << Constants::Network::TCP_PORT << std::endl;
    ...
    while (true) {
        ...
        std::cout << "Client connected for enhanced file transfer." << std::endl;

        // Use FileTransferManager for enhanced receiving
        FileTransferManager transferManager;
        if (transferManager.receiveFile(clientSocket, ".")) {
...
#else
void handle_enhanced_file_transfer_server_linux() {
    int server_fd, new_socket;
    struct sockaddr_in address;
    int opt = 1;
    int addrlen = sizeof(address);
    ...
    while (true) {
        ...
        // Use FileTransferManager for enhanced receiving
        FileTransferManager transferManager;
        if (transferManager.receiveFile(new_socket, ".")) {
            std::cout << "File transfer completed successfully." << std::endl;
        } else {
            std::cerr << "File transfer failed." << std::endl;
        }
...
#ifdef _WIN32
    handle_enhanced_file_transfer_server_windows();
#else
    handle_enhanced_file_transfer_server_linux();
#endif
}
